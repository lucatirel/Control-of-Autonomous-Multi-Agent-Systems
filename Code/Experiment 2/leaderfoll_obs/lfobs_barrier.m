function [h,q,dP] = fcn(u,obs)

Nrobots = size(u,1)/4; %number of robots in simulation
Ds = 2; % minimum distance between two robots
...
DeltaP = [DeltaP_{:}].';
for index=1:size(DeltaP,1)
    DeltaP_x(index) = [DeltaP(index,1)];
    DeltaP_y(index) = [DeltaP(index,2)];
end
DeltaP_x_matr = reshape(DeltaP_x,4,4);
DeltaP_y_matr = reshape(DeltaP_y,4,4);

for upper=1:Nrobots
    DeltaP_x_{upper}=[DeltaP_x_matr(upper,upper+1:end)];
    DeltaP_y_{upper}=[DeltaP_y_matr(upper,upper+1:end)];
    
end
DeltaPx = [ DeltaP_x_{:}].';
DeltaPy = [ DeltaP_y_{:}].';
for idx = 1:size(DeltaPx,1);
    dP(idx) = sqrt(DeltaPx(idx)^2 + DeltaPy(2)^2);
end
dP = dP.'
...
h_RobotsOnly = [cel_{:}].'; %h = cell2sym(cell_).';


%obstacles

Nobs= size(obs)/2; %number of obs in simulation
Radius = 2; %the same values as the one set in the block environment/obstacles
Ds = 0.6; % minimum distance between robots-obs
coder.varsize('obs_support_var');
obs_support_var = obs;

for i=1:Nobs
    
    ob_{i} = obs_support_var(1:2);    
    obs_support_var(1:2) = [];
...
for i=1:Nrobots
   for j=1:Nobs
     DeltaPOb_{i,j} = p_{i}-ob_{j}; 
     DeltaVOb_{i,j} = v_{i}-0; 
     hobs_{i,j} = sqrt(2*(alpha_{i})*(norm(DeltaPOb_{i,j})-((Ds/2)+Radius)))...
     + DeltaPOb_{i,j}.'*DeltaVOb_{i,j}/norm(DeltaPOb_{i,j});
     h_obs_matr(i,j) = hobs_{i,j};
...
h_RobotsObstacles = [];
for row=1:size(h_obs_matr,1)
    h_RobotsObstacles = [h_RobotsObstacles; h_obs_matr(row,:).'];
end

%store the barrier functions together
h = [h_RobotsOnly;h_RobotsObstacles]
